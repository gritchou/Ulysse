...
public class EventQueueServiceSBTest {
    ...
    /**
     * test existing of Event in the EventQueue contains a string parameter
     * resourceType
     * 
     * @throws EventQueueServiceException
     */
    ...
    /**
     * test existing of Event in the EventQueue contains/having a string
     * parameter throwedBy
     * 
     * @throws EventQueueServiceException
     */
    ...
    /**
     * test existing of Event in the EventQueue contains/having an string
     * parameter path
     * 
     * @throws EventQueueServiceException
     */
    ...
    public void testFindEventByDate() throws EventQueueServiceException {
        ...
        c.set(2009, 11, 14, 13, 01, 30);
        Date date1 = c.getTime();
        myEvent1.setDate(date1);

        c.set(2009, 11, 14, 15, 30, 30);
        Date date2 = c.getTime();
        myEvent2.setDate(date2);
    ...
    /**
     * 
     * find event in the EventQueue having Date Between date1 and date2
     * (parameters)
     * 
     * @throws EventQueueServiceException
     */
     ...
     * testDesorderPushDelete() throws EventQueueServiceException {
     * logger.debug("test:Test the desorder");
     * 
     * Event myEvent1 = new Event("/path/resource/", "toto", "resourceType",
     * "read", ""); eqs.deleteEvent(pathQ1, myEvent1);
     * 
     * }
     */
   ...
   /* @Test
     ...
     */
    /*
     * @Test public void testcrossCheckOfFindEvent() throws
     * EventQueueServiceException {
     * logger.debug("test: Test: test the cross check of findEvent(...)"); //
     * eqs.createEventQueue("/eventqueue1"); List<Event> list = new
     * ArrayList<Event>(); String nameOwner = "titi";
     * 
     * Event myEvent1 = new Event("/path/resource/", "toto", "resourceType",
     * "read", ""); Event myEvent2 = new Event("/path/resource/", "titi",
     * "resourceType", "read,write", ""); Event myEvent3 = new
     * Event("/path/resource/", "titi", "resourceType", "commit", "");
     * 
     * eqs.pushEvent(pathQ1, myEvent1); eqs.pushEvent(pathQ1, myEvent2);
     * eqs.pushEvent(pathQ1, myEvent3); Event[] tabEvent =
     * eqs.getEvents(pathQ1);
     * 
     * for (int i = 0; i < tabEvent.length; i++) { if
     * (tabEvent[i].getThrowedBy().equals(nameOwner)) { list.add(tabEvent[i]); }
     * }
     * 
     * assertEquals(list.get(0), myEvent3); assertEquals(list.get(1), myEvent2);
