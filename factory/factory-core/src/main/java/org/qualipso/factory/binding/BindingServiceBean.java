...
public class BindingServiceBean implements BindingService {
	...
	public void bind(FactoryResourceIdentifier identifier, String path) throws BindingServiceException, InvalidPathException, PathNotFoundException,
			PathAlreadyBoundException, IdentifierAlreadyBoundException {
	...
	public String getProperty(String path, String name, boolean inherited) throws InvalidPathException, PathNotFoundException, PropertyNotFoundException {
		...
		throw new PropertyNotFoundException("unable to found a property with name [" + name + "] for path [" + npath + "]" + ((inherited)?" even inherited":"") );
	}
	...
	private Node findNodeByBindedResourceIdentifier(FactoryResourceIdentifier identifier) throws PathNotFoundException {
		List<Node> nodes = em.createQuery("select n from Node n where n.bindedResourceIdentifier = :identifier").setParameter("identifier",
				identifier.serialize()).getResultList();
		if (nodes.size() > 0) {
			return nodes.get(0);
		} else {
