			...
			mockery.checking(new Expectations() {
				{
					...
					oneOf(pep).checkSecurity(with(equal("/profiles/jayblanc")), with(equal("/testnode")), with(equal("read"))); inSequence(sequence1);
					oneOf(binding).lookup(with(equal("/testnode"))); will(returnValue(new FactoryResourceIdentifier("FakeService", "FakeResource", "FakeID"))); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/testnode")), with(equal(FactoryResourceProperty.POLICY_ID)), with(equal(false))); will(returnValue("aFakePolicyID")); inSequence(sequence1);
					oneOf(pap).getPolicy(with(equal("aFakePolicyID"))); will(returnValue(expectedPolicy)); inSequence(sequence1);
					oneOf(notification).throwEvent(with(anEventWithTypeEqualsTo("security.resource.get-security-policy"))); inSequence(sequence1);
				}
			});
    ...
    public void testAddSecurityRule() {
        ...
        try {
			mockery.checking(new Expectations() {
				{
					oneOf(membership).getProfilePathForConnectedIdentifier(); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).lookup(with(equal("/projects/project1"))); will(returnValue(new FactoryResourceIdentifier("FakeService", "FakeResource", "FakeID"))); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).lookup(with(equal("/profiles/jayblanc"))); will(returnValue(new FactoryResourceIdentifier("MembershipService", "Profile", "ProfileID"))); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
				}
			});
			...
			mockery.checking(new Expectations() {
				{
					...
					oneOf(binding).lookup(with(equal("/profiles/jayblanc"))); will(returnValue(new FactoryResourceIdentifier("MembershipService", "Profile", "ProfileID"))); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.POLICY_ID)), with(equal(false))); will(returnValue("aFakePolicyID")); inSequence(sequence1);
					oneOf(pap).getPolicy(with(equal("aFakePolicyID"))); will(returnValue(policy15)); inSequence(sequence1);
					oneOf(pap).updatePolicy(with(equal("aFakePolicyID")), with(equal(expectedPolicy))); inSequence(sequence1);
					oneOf(notification).throwEvent(with(anEventWithTypeEqualsTo("security.resource.add-security-rule"))); inSequence(sequence1);
				}
			});
			...
			mockery.checking(new Expectations() {
				{
					...
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.POLICY_ID)), with(equal(false))); will(returnValue("aFakePolicyID")); inSequence(sequence1);
					oneOf(pap).getPolicy(with(equal("aFakePolicyID"))); will(returnValue(policy17)); inSequence(sequence1);
					oneOf(pap).updatePolicy(with(equal("aFakePolicyID")), with(equal(expectedPolicy))); inSequence(sequence1);
					oneOf(notification).throwEvent(with(anEventWithTypeEqualsTo("security.resource.edit-security-rule"))); inSequence(sequence1);
				}
			});
			...
			mockery.checking(new Expectations() {
				{
					...
					oneOf(pap).getPolicy(with(equal("aFakePolicyID"))); will(returnValue(policy17)); inSequence(sequence1);
					oneOf(pap).updatePolicy(with(equal("aFakePolicyID")), with(equal(expectedPolicy))); inSequence(sequence1);
					oneOf(notification).throwEvent(with(anEventWithTypeEqualsTo("security.resource.remove-security-rule"))); inSequence(sequence1);
				}
			});
			...
			mockery.checking(new Expectations() {
				{
					...
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).lookup(with(equal("/profiles/jayblanc"))); will(returnValue(new FactoryResourceIdentifier("MembershipService", "Profile", "ProfileID"))); inSequence(sequence1);
					oneOf(binding).lookup(with(equal("/profiles/userlambda"))); will(returnValue(new FactoryResourceIdentifier("MembershipService", "Profile", "ProfileIDLambda"))); inSequence(sequence1);
					oneOf(membership).getServiceName(); will(returnValue("MembershipService"));
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal(false))); will(returnValue("/profiles/jayblanc")); inSequence(sequence1);
					oneOf(binding).getProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.POLICY_ID)), with(equal(false))); will(returnValue("aFakePolicyID")); inSequence(sequence1);
					oneOf(pap).getPolicy(with(equal("aFakePolicyID"))); will(returnValue(policy17)); inSequence(sequence1);
					oneOf(pap).updatePolicy(with(equal("aFakePolicyID")), with(equal(expectedPolicy))); inSequence(sequence1);
					oneOf(binding).setProperty(with(equal("/projects/project1")), with(equal(FactoryResourceProperty.OWNER)), with(equal("/profiles/userlambda"))); inSequence(sequence1);
					oneOf(notification).throwEvent(with(anEventWithTypeEqualsTo("security.resource.change-owner"))); inSequence(sequence1);
				}
			});
