    ...
    public void testCRUDIndexing() {
        ...
        try {
            mockery.checking(new Expectations() {
                {
                    oneOf(queueConnectionFactory).createQueueConnection().createQueueSession(true, javax.jms.Session.AUTO_ACKNOWLEDGE).createSender(
                            indexingQueue).send(with(any(Message.class)));
