...
@Stateless(name = "DocumentServiceBean", mappedName = FactoryNamingConvention.JNDI_SERVICE_PREFIX + "DocumentService")
@WebService(endpointInterface = "org.qualipso.factory.collaboration.document.DocumentService", targetNamespace = "http://org.qualipso.factory.ws/service/document", serviceName = "DocumentService", portName = "DocumentServicePort")
@WebContext(contextRoot = "/factory-service-collaboration", urlPattern = "/document")
...
public class DocumentServiceBean implements DocumentService
{
    ...
    private static final String SERVICE_NAME = "DocumentService";
    private static final String[] RESOURCE_TYPE_LIST = new String[] { "Document", "CollaborationFolder" };
	...
	try
	{
		...
		if (caller == null)
		{
		    throw new DocumentServiceException("Could not get connected profile");
		}
		logIt("caller: " + caller);
		String parentPath = "/";
		if (!path.equals("/"))
		...
		{
		    CollaborationFolder parFold = readFolder(parentPath);
		    if (parFold != null)
		    {
		    ...
		    {
			// Should we create it?
			String tempName = "F" + System.currentTimeMillis();
			if (parentPath.indexOf("/") > 0)
			{
			    int i = parentPath.lastIndexOf("/");
			    if (i + 1 < parentPath.length())
			    {
				tempName = parentPath.substring(i);
			    }
			}
			logIt("Create folder before uploading file. Path " + parentPath + " Name " + tempName);
			String folderID = createFolder(parentPath, tempName, "Folder created by factory");
			if (folderID == null)
			{
			    throw new DocumentServiceException(
				    "Error in creating folder for the document upload.Path: " + path);
			}
		...
		HashMap<String, String> values = documentWS.createDocument(parentFolderID, name, date, type, keywords,
			version, status, fileName, mimeType, binaryContent);
		if (values != null && !values.isEmpty())
		{
		    ...
		    if (!code.equals(CollaborationUtils.SUCCESS_CODE) || (newId == null || newId.equals("")))
		    {
			throw new DocumentServiceException("Error code recieved from the WS." + " Code" + code
				+ " Message:" + msg);
		    }
		    ...
		    // Notify
		    notification.throwEvent(new Event(path, caller, "Document", "collaboration.document.create", ""));
		    //
		    logIt(doc.toString());
		} else
		{
		    throw new DocumentServiceException("No valid answer from the WS.Check logs.");
		}
	    }
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to create the name at path " + path, e);
	}
    }
	...
	try
	{
	    ...
	    checkResourceType(identifier, "Document");
	    // Find the entity
	    document = em.find(Document.class, identifier.getId());
	    if (document == null)
	    {
		throw new DocumentServiceException("unable to find a document for id " + identifier.getId());
	    }
	    // Call the WS to retrieve values that are not stored in Mermig CMS
	    HashMap<String, Object> values = documentWS.readDocument(document.getId());
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		logIt("Message Code:" + code + " Message: " + msg);
		if (!code.equals(CollaborationUtils.SUCCESS_CODE) || values.get("document") == null
		...
		DocumentDTO docCont = (DocumentDTO) values.get("documentContent");
		if (docCont != null && docWS != null)
		{
		    ...
		    // Notify
		    notification.throwEvent(new Event(path, caller, "Document", "collaboration.document.read", ""));
		    //
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to read the document at path " + path, e);
	}
	return document;
    }
	...
	try
	{
	    ...
	    // Call the WS to retrieve values that are not stored in Mermig CMS
	    HashMap<String, Object> values = documentWS.readDocumentProperties(document.getId());
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
	...
	{
	    logger.error("unable to read the document properties at path " + path, e);
	    throw new DocumentServiceException("unable to read the document properties at path " + path, e);
	}
	return document;
    }
	...
	try
	{
	    ...
	    HashMap<String, String> values = documentWS.updateDocument(doc.getId(), name, type, keywords, status,
		    fileName, mimeType, binaryContent);
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		...
		binding.setProperty(path, FactoryResourceProperty.LAST_UPDATE_TIMESTAMP, System.currentTimeMillis()
			+ "");
		// notify
		notification.throwEvent(new Event(path, caller, "Document", "collaboration.document.update", ""));
		//
		logIt(doc.toString());
	    } else
	    {
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to update the document at path " + path, e);
	}
    }
	...
	try
	{
	    ...
	    HashMap<String, String> values = documentWS.deleteDocument(doc.getId());
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		...
		binding.unbind(path);
		// Notify
		notification.throwEvent(new Event(path, caller, "Document", "collaboration.document.delete", ""));
	    } else
	    {
		throw new DocumentServiceException("No valid answer from the WS.Check logs.");
	    }
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to delete the document at path " + path, e);
	}
    }
	...
	try
	{
		...
		{
		    logger.warn("Error in reading parent. Probably not a collaboration folder type. Error message: "
			    + e.getMessage());
		}
	    }
	    logIt("Folder id " + parentFolderID);
	    ...
	    HashMap<String, String> values = documentWS.createFolder(name, parentFolderID, description);
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		...
		logIt("Message Code:" + code + " Message: " + msg);
		if (!code.equals(CollaborationUtils.SUCCESS_CODE) || (newFolderId == null || newFolderId.equals("")))
		{
		    throw new DocumentServiceException("Error code recieved from the WS." + " Code" + code
			    + " Message:" + msg);
		}
		...
		binding.setProperty(path, FactoryResourceProperty.POLICY_ID, policyId);
		// Notify
		notification.throwEvent(new Event(path, caller, "CollaborationFolder", "collaboration.folder.create",
			""));
		//
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to create the folder at path " + path, e);
	}
	return path;
    }
	...
	try
	{
	    ...
	    // Look up given path and check resource type
	    FactoryResourceIdentifier identifier = binding.lookup(path);
	    checkResourceType(identifier, "CollaborationFolder");
	    // Find the entity
	    folder = em.find(CollaborationFolder.class, identifier.getId());
	    if (folder == null)
	    {
		throw new DocumentServiceException("unable to find a folder for id " + identifier.getId());
	    }
	    // Call WS getFolderProperties
	    // TODO return folderdto
	    HashMap<String, String> values = documentWS.readFolder(folder.getId());
	    if (values != null && !values.isEmpty())
	    {
	    ...
	    folder.setResourcePath(path);
	    // Notify
	    notification.throwEvent(new Event(path, caller, "CollaborationFolder", "collaboration.folder.read", ""));
	    //
	    logIt(folder.toString());
	} catch (Exception e)
	{
	    logger.error("unable to read the folder at path " + path, e);
	    throw new DocumentServiceException("unable to read the folder at path " + path, e);
	}
	return folder;
    }
	...
	try
	{
	    ...
	    checkResourceType(identifier, "CollaborationFolder");
	    // Find the entity
	    CollaborationFolder folder = em.find(CollaborationFolder.class, identifier.getId());
	    if (folder == null)
	    {
		throw new DocumentServiceException("unable to find a document for id " + identifier.getId());
	    }
	    // Call the WS to update the folder
	    HashMap<String, String> values = documentWS.updateFolder(folder.getId(), name, description);
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		...
		binding.setProperty(path, FactoryResourceProperty.LAST_UPDATE_TIMESTAMP, System.currentTimeMillis()
			+ "");
		// Notify
		notification.throwEvent(new Event(path, caller, "CollaborationFolder", "collaboration.folder.update",
			""));
		//
		logIt(folder.toString());
	    } else
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to update the folder at path " + path, e);
	}
    }
	...
	try
	{
	    ...
	    HashMap<String, String> values = documentWS.deleteFolder(folder.getId());
	    if (values != null && !values.isEmpty())
	    {
		String code = (String) values.get("statusCode");
		String msg = (String) values.get("statusMessage");
		...
		binding.unbind(path);
		// Notify
		notification.throwEvent(new Event(path, caller, "CollaborationFolder", "collaboration.folder.delete",
			""));
	    } else
	    {
		throw new DocumentServiceException("No valid answer from the WS.Check logs.");
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to delete the folder at path " + path, e);
	}
    }
	...
	try
	{
		...
		String servicePattern = "DocumentService";
		String typePattern = "CollaborationFolder";
		String[] foldersArray = browser.listChildrenOfType(path, servicePattern, typePattern);
		if (foldersArray != null && foldersArray.length > 0)
		{
			...
			logIt(cf.toString());
			ListItem li = new ListItem(cf.getId(), cf.getName(), cf.getResourcePath(), "folder");
		...
		String[] docsArray = browser.listChildrenOfType(path, servicePattern, typePattern);
		if (docsArray != null && docsArray.length > 0)
		{
			...
			logIt(dc.toString());
			ListItem li = new ListItem(dc.getId(), dc.getName(), dc.getResourcePath(), "document");
			list.add(li);
		    }
		}
		if (list != null && list.size() > 0)
		{
		    logIt("The number of items found under " + path + " is " + list.size());
		    folder.setContent(list.toArray(new ListItem[list.size()]));
		}
	    } else
	    {
		throw new DocumentServiceException("unable to retrieve folder of path " + path);
	    }

	} catch (Exception e)
	{
	    logger.error("unable to retrieve content folder of path " + path, e);
	    throw new DocumentServiceException("unable to retrieve content folder of path " + path, e);
	}
	return folder;
    }
	...
	try
	{
	    ...
	    {
		logIt("The number of items found under " + path + " is " + listVector.size());
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to find documents for path " + path, e);
	}
	return docList;
    }
	...
	} catch (Exception e)
	{
	    ...
	    throw new DocumentServiceException("unable to find subfolders for path " + path, e);
	}
	return folderList;
    }
	...
	try
	{
	    ...
	    if (identifier.getType().equals("Document"))
	    {
